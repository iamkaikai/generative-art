...
let walkY = 0;
let StartY = 0;
let len = randBetweenInt(30,randBetweenInt(60,200));
let runTime = 1;
let offVolume = randBetween(10,20);
let colorGroup = randBetweenInt(0,119);
let length,spacing;
let case1length = randBetweenInt(2,14);
let case1walkX = randBetweenInt(0,30);
let case1alpha = randBetween(0.7,0.9);
let case2length = randBetweenInt(8,12);
let case2height = randBetween(0.01,0.04);
let case3height = randBetween(0.01,0.08);
let case2pattern = randBetweenInt(0,1);
let case2randY = 1.2;
let case2ColorGroup = randBetweenInt(0,119);
let case3length = randBetweenInt(4,6);
let case4pattern = randBetween(0,1);
let case1randY = randBetween(0.95,1.05)
let case3randY = randBetween(0.92,1.07)
let case4randY = randBetween(0.92,0.95);
let case5randY = randBetween(0.7,0.9);
let noiseD = 2;
let case1Radius = randBetweenInt(1,60);
...
function setup() {
	...
	for(let x=0; x<=width; x++){
		noiseA.push([]);
		for(let y=0; y<=height/noiseD; y++){
			noiseA[x].push(randBetweenInt(0,1));
		}
	}
	background(bg[colorGroup]);
}
...
function draw() {
	...
	switch(pattern){
			...
			case 0:       
				...
				spacing = length/16;
				strokeWeight(1);
				for(let y=StartY; y<height; y+=length+spacing){
					let offsetX, offsetY;		
					c = round(noise(x*0.01+frameCount*0.01+y)*8);
					stroke(strokeColor[colorGroup][c]);
					offsetX = sin(frameCount*0.005+y*3)*offVolume;
					offsetY = cos(frameCount*0.005+y*3)*offVolume*0.5;
					line(x+offsetX,y+offsetY, x+length+offsetX, y+length+offsetY);
			...
			case 1:
				...
				strokeWeight(1);
				for(let y=StartY; y<n; y+=n2){

					c = round(noise(frameCount*0.03+y)*8);
					console.log("c: "+c);
					offsetX = sin(frameCount*0.005+y*0.005*cos(frameCount*0.001))*offVolume;
					// offsetY = case1length+y*0.0001+x*sin(frameCount*0.001)*0.08;
					offsetY = case1length+y*0.0001+x*sin(frameCount*0.0001)*0.08;
					stroke(colorAlpha(strokeColor[round(noise(x*0.0001+y*0.1)*119)][c],case1alpha));
					line(x+offsetX, y+walkY-offsetY, x+offsetX, y+offsetY*2+walkY-spacing);
				}			
				x += .5;	
				if(x>width+100){
					frameStop(frameCount);									//this
			...
			case 2:
				...
				length = len/case2length;
				spacing = length/2;
				strokeWeight(1);
				for(let y=StartY; y<height-StartY*1.4+6; y+=length+spacing){
					let offsetX, offsetY;
					c = round(noise(frameCount*0.003+y)*8);
					offsetX = sin(frameCount*0.005+y^1.2*abs(sin(y*0.01)))*offVolume;
					if(case2pattern == 0){
						offsetY = -(y*case2height + cos(frameCount*0.005+y^1.1*abs(sin(y)))*offVolume*case2randY);						
					}else{
						offsetY = y*case2height + cos(frameCount*0.005+y^1.1*abs(sin(y)))*offVolume*case2randY;						
					}

					c1 = colorAlpha(strokeColor[round(noise(x*0.001+y)*case2ColorGroup)][c],0.88);
					stroke(c1);
					line(x+offsetX,y-offsetY, x+offsetX, y+offsetY);
						
				}			
				x += 0.05+sin(noise(frameCount*0.001));
			...
			case 3:
				...
				length = len/case3length;
				spacing = length/8;
				strokeWeight(1);
				for(let y=StartY; y<height-shiftX; y+=length+spacing){
					let offsetX, offsetY;
					c = round(noise(frameCount*0.003+y)*8);
					offsetX = sin(frameCount*0.005+y^1.2*abs(sin(y)))*offVolume;
					offsetY = cos(frameCount*0.005+y^1.1*abs(sin(y)))*offVolume*case3randY;
					stroke(strokeColor[colorGroup][c]);										
					line(x+offsetX,y-offsetY, x+offsetX, y+offsetY);
			...
			case 4:
				...
				for(let y=StartY; y<height; y+=length+spacing){
					...
					c = round(noise(frameCount*0.003+y)*8);
					offsetX = sin(frameCount*0.005+y^1.2*abs(sin(y)))*offVolume;
					offsetY = cos(frameCount*0.005+y^1.1*abs(sin(y)))*offVolume*case4randY;
					if(case4pattern < 0.33){
							stroke(noise(frameCount*0.003+y)*255);
					}else{
							stroke(strokeColor[colorGroup][c])
					}
					line(x+offsetX,y-offsetY*0.9, x-offsetX, y+offsetY);
			...
			case 5:
				...
				strokeWeight(1);
				for(let y=StartY; y<height-StartY; y+=length+spacing){
					let offsetX, offsetY;
					c = round(noise(frameCount*0.003+y)*8);
					offsetX = x*0.01+sin(frameCount*0.005+y^1.2*abs(sin(y)))*offVolume;
					offsetY = cos(frameCount*0.005+y^1.1*abs(sin(y)))*offVolume*case5randY;
					if(case4pattern < 0.33){
							stroke(noise(frameCount*0.003+y)*255);
					}else{
							stroke(strokeColor[colorGroup][c])
...
function autoSave(){
	save(canvas, 'image-pattern-'+pattern+'.png');
	setTimeout(() => {
		clear();
		frameCount = 0;
	}, 1000);
